...
function auth(msg: Uint8Array, key: Uint8Array) {
...
function hmac(msg: Uint8Array, key: Uint8Array): Uint8Array {
    const buf = new Uint8Array(BLOCK_SIZE + Math.max(HASH_SIZE, msg.length));
    let i, innerHash;
...
const auth_full: (message: Uint8Array, key: Uint8Array) => Uint8Array = hmac;

const enum BoxLength {
...
function box_before(publicKey: Uint8Array, secretKey: Uint8Array): Uint8Array {
    checkArrayTypes(publicKey, secretKey);
    checkBoxLengths(publicKey, secretKey);

    const k = new Uint8Array(BoxLength.SharedKey);
...
const box_after: (msg: Uint8Array, nonce: Uint8Array, key: Uint8Array) => Uint8Array = secretbox;

function box_open(msg: Uint8Array, nonce: Uint8Array, publicKey: Uint8Array, secretKey: Uint8Array): Uint8Array | undefined {
    const k = box_before(publicKey, secretKey);

    return secretbox_open(msg, nonce, k);
}

const box_open_after: (box: Uint8Array, nonce: Uint8Array, key: Uint8Array) => Uint8Array | undefined = secretbox_open;

function box_keyPair(): BoxKeyPair {
    const pk = new Uint8Array(BoxLength.PublicKey);
    const sk = new Uint8Array(BoxLength.SecretKey);
...
function box_keyPair_fromSecretKey(secretKey: Uint8Array): BoxKeyPair {
...
// low level
function _box_keypair(y: Uint8Array, x: Uint8Array) {
    _randomBytes(x, 32);

    return _scalarMult_base(y, x);
}

function _box_beforenm(k: Uint8Array, y: Uint8Array, x: Uint8Array) {
...
function checkArrayTypes(...arrays: Uint8Array[]) {
...
function encodeUTF8(a: Uint8Array): string {
...
function encodeBase64(a: Uint8Array): string {
    return btoa(fromCharCode.apply(undefined, a as any));
}
...
function encodeHex(a: Uint8Array): string {
    ...
    return fromCharCode.apply(undefined, b as any);
}
...
function decodeHex(s: string): Uint8Array {
    ...
    const a: Uint8Array = new Uint8Array(s.length >> 1);
...
function hash(msg: Uint8Array): Uint8Array {
    ...
    const h = new Uint8Array(HashLength.Hash);
...
function _hash(out: Uint8Array, m: Uint8Array, n: number) {
...
function _hashblocks_hl(hh: Int32Array, hl: Int32Array, m: Uint8Array, n: number): number {
...
function poly1305_init(key: Uint8Array): Poly1305 {
...
function poly1305_blocks(self: Poly1305, m: Uint8Array, mpos: number, bytes: number) {
...
function poly1305_finish(self: Poly1305, mac: Uint8Array, macpos: number) {
...
function poly1305_update(self: Poly1305, m: Uint8Array, mpos: number, bytes: number) {
    let i, want;
...
function _randomBytes(x: Uint8Array, n: number) {
...
function _salsa20(o: Uint8Array, p: Uint8Array, k: Uint8Array, c: Uint8Array) {
    const
...
function _hsalsa20(o: Uint8Array, p: Uint8Array, k: Uint8Array, c: Uint8Array) {
    const
        j0 = c[0] & 0xff | (c[1] & 0xff) << 8 | (c[2] & 0xff) << 16 | (c[3] & 0xff) << 24,
        j1 = k[0] & 0xff | (k[1] & 0xff) << 8 | (k[2] & 0xff) << 16 | (k[3] & 0xff) << 24,
        j2 = k[4] & 0xff | (k[5] & 0xff) << 8 | (k[6] & 0xff) << 16 | (k[7] & 0xff) << 24,
...
const _sigma: Uint8Array = new Uint8Array([101, 120, 112, 97, 110, 100, 32, 51, 50, 45, 98, 121, 116, 101, 32, 107]);
// "expand 32-byte k"

function _stream_salsa20_xor(c: Uint8Array, cpos: number, m: Uint8Array, mpos: number, b: number, n: Uint8Array, k: Uint8Array) {
...
function _stream_salsa20(c: Uint8Array, cpos: number, b: number, n: Uint8Array, k: Uint8Array) {
    const z = new Uint8Array(16), x = new Uint8Array(64);
    let u, i;
    for (i = 0; i < 16; i++) z[i] = 0;
    for (i = 0; i < 8; i++) z[i] = n[i];
...
function _stream(c: Uint8Array, cpos: number, d: number, n: Uint8Array, k: Uint8Array) {
    const s = new Uint8Array(32), sn = new Uint8Array(8);
...
function _stream_xor(c: Uint8Array, cpos: number, m: Uint8Array, mpos: number, d: number, n: Uint8Array, k: Uint8Array) {
    const s = new Uint8Array(32), sn = new Uint8Array(8);
...
const enum ScalarLength {
    Scalar = 32, // scalar bytes
    GroupElement = 32, // bytes
}

function scalarMult(n: Uint8Array, p: Uint8Array) {
    checkArrayTypes(n, p);
    ...
    const q = new Uint8Array(ScalarLength.GroupElement);
...
function scalarMult_base(n: Uint8Array) {
    checkArrayTypes(n);
...
function _scalarMult(q: Uint8Array, n: Uint8Array, p: Uint8Array): 0 {
...
function _scalarMult_base(q: Uint8Array, n: Uint8Array): 0 {
    return _scalarMult(q, n, _9);
}

const enum SealedBoxLength {
...
function sealedbox(m: Uint8Array, pk: Uint8Array): Uint8Array {
    const c = new Uint8Array(SealedBoxLength.Overhead + m.length);
...
function sealedbox_open(c: Uint8Array, pk: Uint8Array, sk: Uint8Array): Uint8Array | undefined {
...
function nonce_gen(pk1: Uint8Array, pk2: Uint8Array) {
...
function secretbox(msg: Uint8Array, nonce: Uint8Array, key: Uint8Array): Uint8Array {
    ...
    const m = new Uint8Array(SecretBoxLength.Zero + msg.length);
...
function secretbox_open(box: Uint8Array, nonce: Uint8Array, key: Uint8Array): Uint8Array | undefined {
    checkArrayTypes(box, nonce, key);
    checkLengths(key, nonce);

    const c = new Uint8Array(SecretBoxLength.Overhead + box.length);
...
// low level
function _secretbox(c: Uint8Array, m: Uint8Array, d: number, n: Uint8Array, k: Uint8Array): -1 | 0 {
    if (d < 32) return -1;
...
function _secretbox_open(m: Uint8Array, c: Uint8Array, d: number, n: Uint8Array, k: Uint8Array): -1 | 0 {
...
function _onetimeauth(out: Uint8Array, outpos: number, m: Uint8Array, mpos: number, n: number, k: Uint8Array): 0 {
...
function _onetimeauth_verify(h: Uint8Array, hpos: number, m: Uint8Array, mpos: number, n: number, k: Uint8Array): number {
...
function sign(msg: Uint8Array, secretKey: Uint8Array): Uint8Array {
    ...
    const signedMsg = new Uint8Array(SignLength.Signature + msg.length);
...
function sign_open(signedMsg: Uint8Array, publicKey: Uint8Array): Uint8Array | undefined {
...
function sign_detached(msg: Uint8Array, secretKey: Uint8Array): Uint8Array {
    const signedMsg = sign(msg, secretKey);

    const sig = new Uint8Array(SignLength.Signature);
...
function sign_detached_verify(msg: Uint8Array, sig: Uint8Array, publicKey: Uint8Array): boolean {
    ...
    const sm = new Uint8Array(SignLength.Signature + msg.length);
    const m = new Uint8Array(SignLength.Signature + msg.length);
...
function sign_keyPair(): SignKeyPair {
    const pk = new Uint8Array(SignLength.PublicKey);
    const sk = new Uint8Array(SignLength.SecretKey);
...
function sign_keyPair_fromSecretKey(secretKey: Uint8Array): SignKeyPair {
    checkArrayTypes(secretKey);
...
function sign_keyPair_fromSeed(seed: Uint8Array): SignKeyPair {
...
function _verify_16(x: Uint8Array, xi: number, y: Uint8Array, yi: number): number {
    return vn(x, xi, y, yi, 16);
}

function _verify_32(x: Uint8Array, xi: number, y: Uint8Array, yi: number): number {
...
export {secretbox, secretbox_open};
